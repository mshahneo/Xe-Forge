...
//
// This kernel (@payload_kernel) differs from the vector-dialect FMHA in
// test/issue_1824 in three important ways, all mirrored by the CPU reference:
//   1. The whole online softmax runs in f16 (running max/sum/acc are f16
//      vectors), so the reference emulates f16 rounding at every point the
//      kernel materialises an f16 value.  dpas accumulates internally in f32,
//      which the reference matches with f32 dot-product accumulators.
//   2. It uses NATURAL math.exp (no exp2 / log2e factor).
//   3. The scale (0.125 = 1/sqrt(64)) is a baked-in constant applied to the
//      f16 Q*K^T result: S_scaled = truncf(S_f16 * 0.125).
//   4. The correction factor is computed as exp(-m_new)/exp(-m_old)
//      (== exp(m_old - m_new)); on the first block m_old = -inf so
//      exp(-m_old) = +inf and the correction is 0 (acc/l start at 0).
//
...
module @fa attributes {gpu.container_module} {
  gpu.module @payload_kernel [#xevm.target<O = 3>] {
    gpu.func @payload_kernel(%arg0: memref<16x4096x64xf16>, %arg1: memref<2x8x4096x64xf16>, %arg2: memref<2x8x4096x64xf16>, %arg3: memref<2x8x4096x64xf16>, %arg4:memref<2x8x4096x64xf16>) kernel attributes {known_block_size = array<i32: 128, 1, 1>, known_grid_size = array<i32: 16, 32, 1>} {
      ...
      %cst = arith.constant dense<1.250000e-01> : vector<128x64xf16>
      %c8 = arith.constant 8 : index
      %cst_0_f32 = arith.constant dense<0.000000e+00> : vector<128xf32>
%cst_0 = arith.constant dense<0.000000e+00> : vector<128xf16>
      %cst_1_f32 = arith.constant dense<0.000000e+00> : vector<128x64xf32>
%cst_1 = arith.constant dense<0.000000e+00> : vector<128x64xf16>
      %c0 = arith.constant 0 : index
      %cst_2_f32 = arith.constant dense<0xFF800000> : vector<128xf32>
%cst_2 = arith.constant dense<0xFC00> : vector<128xf16>
      ...
      %18:3 = scf.for %arg5 = %c0 to %c4096 step %c64 iter_args(%arg6 = %cst_0_f32, %arg7 = %cst_1_f32, %arg8 = %cst_2_f32) -> (vector<128xf32>, vector<128x64xf32>, vector<128xf32>) {
        ...
        %32 = xegpu.load_nd %13[%arg5, 0] <{layout = #xegpu.layout<sg_layout = [8, 1], sg_data = [64, 64], inst_data = [16, 16]>}> : !xegpu.tensor_desc<64x64xf16,#xegpu.block_tdesc_attr<boundary_check = false>> -> vector<64x64xf16>
        %33 = vector.transpose %32, [1, 0] : vector<64x64xf16> to vector<64x64xf16>
        %34_f32 = xegpu.dpas %31, %33, %cst_1_f32 <{layout_a = #xegpu.layout<sg_layout = [8, 1], sg_data = [16, 64], inst_data = [8, 16]>, layout_b = #xegpu.layout<sg_layout = [1, 8], sg_data = [64, 64], inst_data = [16, 16], order = [0, 1]>, layout_cd = #xegpu.layout<sg_layout = [8, 1], sg_data = [16, 64], inst_data = [8, 16]>}> : vector<128x64xf16>, vector<64x64xf16>, vector<128x64xf32> -> vector<128x64xf32>
        %35_f32 = arith.mulf %34_f32, %cst : vector<128x64xf32>
        %36_f32 = vector.multi_reduction <maximumf>, %35_f32, %arg8 [1] : vector<128x64xf32> to vector<128xf32>
        %37_f32 = arith.subf %arg8, %36_f32 fastmath<afn> : vector<128xf32>
        %38_f32 = math.exp %37_f32 fastmath<afn> : vector<128xf32>
        %42_f32 = vector.broadcast %38_f32 : vector<128xf32> to vector<64x128xf32>
        %43_f32 = vector.transpose %42_f32, [1, 0] : vector<64x128xf32> to vector<128x64xf32>
        %44_f32 = arith.mulf %arg7, %43_f32 fastmath<afn> : vector<128x64xf32>
        %45_f32 = vector.broadcast %36_f32 : vector<128xf32> to vector<64x128xf32>
        %46_f32 = vector.transpose %45_f32, [1, 0] : vector<64x128xf32> to vector<128x64xf32>
        %47_f32 = arith.subf %35_f32, %46_f32 fastmath<afn> : vector<128x64xf32>
        %48_f32 = math.exp %47_f32 fastmath<afn> : vector<128x64xf32>
        %49 = xegpu.load_nd %17[%arg5, 0] <{layout = #xegpu.layout<sg_layout = [8, 1], sg_data = [64, 64], inst_data = [16, 16]>}> : !xegpu.tensor_desc<64x64xf16,#xegpu.block_tdesc_attr<boundary_check = false>> -> vector<64x64xf16>
        %48_f16 = arith.truncf %48_f32 : vector<128x64xf32> to vector<128x64xf16>
%50_f32 = xegpu.dpas %48_f16, %49, %44_f32 <{layout_a = #xegpu.layout<sg_layout = [8, 1], sg_data = [16, 64], inst_data = [8, 16]>, layout_b = #xegpu.layout<sg_layout = [8, 1], sg_data = [64, 64], inst_data = [16, 16]>, layout_cd = #xegpu.layout<sg_layout = [8, 1], sg_data = [16, 64], inst_data = [8, 16]>}> : vector<128x64xf16>, vector<64x64xf16>, vector<128x64xf32> -> vector<128x64xf32>
        %51_f32 = arith.mulf %arg6, %38_f32 fastmath<afn> : vector<128xf32>
        %52_f32 = vector.multi_reduction <add>, %48_f32, %51_f32 [1] : vector<128x64xf32> to vector<128xf32>
        scf.yield %52_f32, %50_f32, %36_f32 : vector<128xf32>, vector<128x64xf32>, vector<128xf32>
      }
      %19_f32 = vector.broadcast %18#0 : vector<128xf32> to vector<64x128xf32>
      %20_f32 = vector.transpose %19_f32, [1, 0] : vector<64x128xf32> to vector<128x64xf32>
      %21_f32 = arith.divf %18#1, %20_f32 fastmath<afn> : vector<128x64xf32>
%21 = arith.truncf %21_f32 : vector<128x64xf32> to vector<128x64xf16>
      %22 = arith.floordivsi %block_id_x, %c8 : index
      %23 = arith.remsi %block_id_x, %c8 : index
      %24 = arith.cmpi slt, %23, %c0 : index
      %25 = arith.addi %23, %c8 overflow<nsw> : index
      %26 = arith.select %24, %25, %23 : index
      %subview_15 = memref.subview %arg4[%22, %26, 0, 0] [1, 1, 4096, 64] [1, 1, 1, 1] : memref<2x8x4096x64xf16> to memref<4096x64xf16, strided<[64, 1], offset: ?>>
